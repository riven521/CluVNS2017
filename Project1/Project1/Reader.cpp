...
CluVRPinst* Reader::read(std::string filePathInstance)
{
	...
	if (setSelection_ == 3 || setSelection_ == 4 || setSelection_ == 5 || setSelection_ == 6 || setSelection_ == 7)
	{
		...
		std::vector<int> vid;
		std::vector<int> vclusterid;
		for (int i = 0; i < dimension; i++)
		{
			fscanf_s(f_ptr, "%d %d", &id, &clusterid);
			vid.push_back(id);
			vclusterid.push_back(clusterid);
		}
		nClusters = vclusterid[0];
		...
		{
			//fscanf_s(f_ptr, "%d %d", &id, &clusterid);
			vNodes.at(vid[i] - 1).cluster = vclusterid[i] - 1;
			vClusters.at(vclusterid[i] - 1).addNode(vNodes.at(vid[i] - 1));
			vClusters.at(vclusterid[i] - 1).addDemand(vDemands.at(vid[i] - 1));
		}
		...
		nVehicles = (int)ceil((double)sumDemand / (double)Globals::vehicleCapacity);	//若算例没有给定车辆数,使用总需求车辆容量后向上取整
	}
